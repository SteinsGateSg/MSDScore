...
model_id = os.environ.get("BLIP_MODEL_ID", "Salesforce/blip-itm-base-coco")
json_path = os.environ.get("SUGARCREPE_JSON", "data/SugarCrepe/add_att.json")
...
def get_image_features(image):
    ...
    with torch.no_grad():
        ...
        img_global = image_features[:, 0, :]
        img_patch = image_features[:, 1:, :]
...
def get_text_features(text):
    ...
    with torch.no_grad():
        ...
        txt_global = text_features[:, 0, :]
        valid_len = inputs.attention_mask.sum().item()
        if valid_len > 2:
            txt_token = text_features[:, 1:valid_len-1, :]
        else:
            txt_token = text_features[:, :valid_len, :]
